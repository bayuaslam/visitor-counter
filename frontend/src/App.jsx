...
function LaboranInventory({ role }) {
  ...
  return <section className="panel inventory-manager">
    ...
    <div className="inventory-table"><div className="inventory-table-row inventory-table-header"><span>Kode</span><span>Nama alat</span><span>Kategori</span><span>Stok</span><span>Aksi</span></div>{items.length === 0 && <div className="inventory-table-empty"><strong>Inventory masih kosong</strong><span>Tambahkan alat atau import Excel. Tombol Edit dan Hapus akan muncul pada setiap baris alat.</span></div>}{items.slice(0, 12).map((item) => <div className="inventory-table-row" key={item.id}><span>{item.asset_code}</span><strong>{item.name}</strong><span>{item.category}</span><span>{item.quantity_available}/{item.quantity_total}</span><span className="row-actions"><button onClick={() => startEdit(item)} aria-label={`Edit ${item.name}`}><Pencil size={13} /> Edit</button><button className="delete-action" onClick={() => remove(item)} aria-label={`Hapus ${item.name}`}><Trash2 size={13} /> Hapus</button></span></div>)}</div>
    <p className="import-hint">Gunakan Edit untuk mengganti total unit, jumlah tersedia, kondisi, lokasi, atau detail alat. Kolom Excel: asset_code, name, category, quantity_total, quantity_available, condition, location, description.</p>
  </section>;
}
...
function NotificationCenter({ admin }) {
  const role = admin ? "LABORAN" : "STUDENT";
  const [open, setOpen] = useState(false);
  const [data, setData] = useState({ unread: 0, items: [] });

  const load = useCallback(async () => {
    const response = await fetch("/api/notifications?limit=20", { headers: apiHeaders(role), cache: "no-store" });
  ...
  const readOne = async (item) => {
    if (!item.read) await fetch(`/api/notifications/${item.id}/read`, { method: "PATCH", headers: apiHeaders(role) });
    await load();
  };

  const readAll = async () => {
    await fetch("/api/notification-actions/read-all", { method: "PATCH", headers: apiHeaders(role) });
    await load();
  };

  return <div className="notification-center">
    <button className="icon-button notification-trigger" onClick={() => setOpen((current) => !current)} aria-label={`Notifikasi${data.unread ? `, ${data.unread} belum dibaca` : ""}`} aria-expanded={open}><Bell size={20} />{data.unread > 0 && <span className="notification-badge">{data.unread > 99 ? "99+" : data.unread}</span>}</button>
    {open && <div className="notification-popover">
      <div className="notification-head"><div><strong>Notifikasi</strong><span>{data.unread} belum dibaca</span></div>{data.unread > 0 && <button onClick={readAll}>Tandai semua dibaca</button>}</div>
      <div className="notification-list">{data.items.length === 0 && <div className="notification-empty"><Bell size={22} /><strong>Belum ada notifikasi</strong><span>Update pengajuan akan tampil di sini.</span></div>}{data.items.map((item) => <button className={`notification-item ${item.read ? "notification-read" : ""}`} key={item.id} onClick={() => readOne(item)}><span className="notification-dot" /><div><strong>{item.title}</strong><p>{item.message}</p><small>{new Date(item.created_at).toLocaleString("id-ID")}</small></div></button>)}</div>
    </div>}
  </div>;
}

function App() {
  const [adminSurface, setAdminSurface] = useState(() => window.location.pathname.startsWith("/admin"));
  const [page, setPage] = useState(() => window.location.pathname.startsWith("/admin") ? "laboran" : "dashboard");
  ...
  useEffect(() => {
    const syncPath = () => {
      ...
      setPage(admin ? "laboran" : "dashboard");
    };
  ...
  const navigate = (nextPage) => {
    const nextIsAdmin = nextPage === "laboran";
    if (nextIsAdmin !== adminSurface) {
      window.history.pushState({}, "", nextIsAdmin ? "/admin" : "/");
      setAdminSurface(nextIsAdmin);
    }
    setPage(nextPage);
    setMenuOpen(false);
    ...
    <div className="app-shell">
      <aside className={`sidebar ${menuOpen ? "sidebar-open" : ""}`}>
        ...
        <nav aria-label="Navigasi utama">
          ...
          {adminSurface ? <><button className="nav-active" onClick={() => navigate("laboran")}><LayoutDashboard size={19} /><span>Dashboard Laboran</span></button><button onClick={() => navigate("dashboard")}><Home size={19} /><span>Dashboard</span></button></> : navItems.map(({ id, label, icon: Icon, upcoming }) => (
      ...
      <main>
        <header className="topbar">
          ...
          <div className="top-actions"><NotificationCenter admin={adminSurface} /><div className="avatar">{adminSurface ? "LB" : "MH"}</div></div>
        </header>
      ...
      <nav className={`mobile-nav ${adminSurface ? "admin-mobile-nav" : ""}`} aria-label="Navigasi mobile">
        {adminSurface ? <><button className="mobile-active" onClick={() => navigate("laboran")}><LayoutDashboard size={20} /><span>Laboran</span></button><button onClick={() => navigate("dashboard")}><Home size={20} /><span>Umum</span></button></> : <><button className={page === "dashboard" ? "mobile-active" : ""} onClick={() => navigate("dashboard")}><Home size={20} /><span>Home</span></button><button className={page === "equipment" ? "mobile-active" : ""} onClick={() => navigate("equipment")}><Box size={20} /><span>Alat</span></button><button className={page === "rooms" ? "mobile-active" : ""} onClick={() => navigate("rooms")}><CalendarDays size={20} /><span>Ruangan</span></button></>}
      </nav>
    </div>
  );
}
